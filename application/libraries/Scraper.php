<?php require_once('phpwhois-4.2.2/whois.main.php'); // including 
	...
	*/
    ...
    foreach ($matches[11] as $match) {
	    ...
	    * @param string
	    */
        private function getContents($url, $proxy='',$string_replace_info=array())
        {
			...
			for($i=33;$i<=127;$i++){
				$html_encoded="&#".$i.";";
				$html_decode=chr($i);
				$buffer=str_replace($html_encoded,$html_decode,$buffer);
				
			}
			
			/******if decode email string is set then replace the decode string ******/
			
			if(!empty($string_replace_info)){
				
				foreach($string_replace_info as $replace_info){
					$search_string=$replace_info['search_string'];
					$replaced_by=$replace_info['replaced_by'];
					$buffer= str_replace($search_string,$replaced_by,$buffer);
				}
			}
    	...
    	* @param string
    	*/    
        public function start_scrapping($proxy='',$string_replace_info=array())
        {
            $scrapping_url=$this->scrape_url;
            $content=$this->getContents($scrapping_url, $proxy,$string_replace_info);
            ...
            if ($this->scrape_url && $this->full_site_scrape==1) {
                $this->start_scrapping($proxy,$string_replace_info);
            } else {
	    ...
	    */
        public function getContentFromSearchEngine($url, $proxy='')
        {
            ...
            curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);// allow redirects
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1); // return into a variable
            curl_setopt($ch, CURLOPT_TIMEOUT, 5000); // times out after 50s
            curl_setopt($ch, CURLOPT_POST, 0); // set POST method
	    ...
	    */
        public function googleSearch($keyword, $page_number=0, $proxy='')
        {
			...
			if($proxy=="")
				...
				$url="www.google.com/search?q={$keyword}&num=100&ie=utf-8&oe=utf-8{$page_str}";
	    ...
	    * @param string
	    */
        public function get_email_from_url($url, $proxy='',$string_replace_info=array())
        {
            $content=$this->getContents($url, $proxy,$string_replace_info);
            /***Get all email address in this page**/
            $found_email=$this->get_email($content);
